...
export default function DestinationMap({ coords, className, landmarks = [], activeLocation }) {
  ...
  useEffect(() => {
    if (coords && coords.length === 2 && coords[0] !== 0) {
      ...
      setMapCenter([parseFloat(landmarks[0].lat), parseFloat(landmarks[0].lng)]);
    }
  }, [coords, landmarks]);
  ...
  if (!mapCenter) {
    return (
      <div className={`flex items-center justify-center bg-[#050505] ${className}`}>
  ...
  return (
    <div className={`relative z-0 ${className}`}>
      ...
      >
        ...
        <MapController activeLocation={activeLocation} defaultCenter={mapCenter} />
        ...
        {landmarks.map((place, index) => {
          ...
          return (
            <Marker key={index} position={[parseFloat(place.lat), parseFloat(place.lng)]} icon={customOrangeIcon}>
              <Popup className="custom-popup text-black">
                <div className="p-1">
                  <h3 className="font-bold text-sm uppercase mb-1">{place.name}</h3>
                  <p className="text-xs text-gray-600 m-0 leading-tight">{place.desc}</p>
                </div>
              </Popup>
            </Marker>
          );
